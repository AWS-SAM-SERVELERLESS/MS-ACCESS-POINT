...
```bash
access-ponit$ sam local invoke baseFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
access-ponit$ sam local start-api
access-ponit$ curl http://localhost:3000/
```
...
```bash
access-ponit$ sam logs -n baseFunction --stack-name access-ponit --tail
```
...
```bash
sam delete --stack-name access-ponit
```
